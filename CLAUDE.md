...
2. 형식:
   ...
   English body
   ```
3. `git add public/desk-posts/[slug].md && git commit && git push`
4. 푸시만으로는 반영되지 않습니다. 위 **Deployment** 절에 따라 이미지를 굽고 VM 유닛 `cafelua.service` 의 IMAGE 를 새 태그로 바꿔 재시작해야 글이 올라갑니다.
