// Generated by scripts/publish-fstory-homepages.mjs — do not edit by hand.
// Source of truth: scripts/fstory-lineage.mjs

export type FstoryLineageId = 'L0' | 'L1' | 'L2.0' | 'L2.1' | 'L2.2' | 'L3';

export type FstorySnapshotId = '20010715123146' | '20010723051951' | '20010925220320' | '20011202212712' | '20020325014505' | '20020924164928' | '20021120053627' | '20021128181318' | '20030726202839';

export type FstoryLineage = {
    id: FstoryLineageId;
    label: string;
    period: string;
    representative: FstorySnapshotId;
    summary: string;
};

export type FstorySnapshot = {
    timestamp: FstorySnapshotId;
    date: string;
    lineage: FstoryLineageId;
    description: string;
};

export const FSTORY_LINEAGES: FstoryLineage[] = [
    {
        "id": "L0",
        "label": "도메인 호스팅 안내",
        "period": "2001-07-15",
        "representative": "20010715123146",
        "summary": "도메인 등록 대행사의 기본 안내 화면. 홈페이지 본체가 아니다."
    },
    {
        "id": "L1",
        "label": "클래식 (1998 ~ 2001-07)",
        "period": "1998 ~ 2001-07-23",
        "representative": "20010723051951",
        "summary": "프레임 없이 title1~4.gif 메뉴 그림을 세로로 늘어놓은 초기 계보. Wayback에 남은 마지막 상태가 2001-07-23이다."
    },
    {
        "id": "L2.0",
        "label": "ver 2.0 최초판",
        "period": "2001-09 ~ 2002-03",
        "representative": "20011202212712",
        "summary": "index.html 안의 800x600 iframe이 main.html(42/560 2분할 프레임)을 부른다. 메뉴에 ai·study 항목이 있다."
    },
    {
        "id": "L2.1",
        "label": "ver 2.0 메뉴 개편판",
        "period": "2002-03 ~ 2002-11-20",
        "representative": "20021120053627",
        "summary": "프레임 구조는 그대로이고 메뉴가 teatime·tech·zboard 중심으로 바뀐다. 본문 첫 화면이 diary/diary.html이다."
    },
    {
        "id": "L2.2",
        "label": "ver 2.0 5분할 재설계판",
        "period": "2002-11-28 ~ 2003-07",
        "representative": "20021128181318",
        "summary": "main.html이 topmenu·left·content·right·bottom 5분할 프레임으로 바뀌고 bgm 프레임이 붙는다. album·cartoon·media·insidece 메뉴가 새로 생기고 싸이월드 링크가 처음 등장한다."
    },
    {
        "id": "L3",
        "label": "ver 3.0 싸이월드 연동판",
        "period": "2003-07-26",
        "representative": "20030726202839",
        "summary": "프레임을 모두 걷어내고 단일 index.html로 돌아간 마지막 판. 본문 대부분을 싸이월드 미니홈피에 넘겼다."
    }
];

export const FSTORY_SNAPSHOTS: FstorySnapshot[] = [
    {
        "timestamp": "20010715123146",
        "date": "2001-07-15",
        "lineage": "L0",
        "description": "도메인 호스팅 초기 화면"
    },
    {
        "timestamp": "20010723051951",
        "date": "2001-07-23",
        "lineage": "L1",
        "description": "클래식 계보의 마지막 확인 상태"
    },
    {
        "timestamp": "20010925220320",
        "date": "2001-09-25",
        "lineage": "L2.0",
        "description": "ver 2.0 최초 확인"
    },
    {
        "timestamp": "20011202212712",
        "date": "2001-12-02",
        "lineage": "L2.0",
        "description": "ver 2.0 · 본문 프레임까지 확보"
    },
    {
        "timestamp": "20020325014505",
        "date": "2002-03-25",
        "lineage": "L2.1",
        "description": "메뉴 개편 직후"
    },
    {
        "timestamp": "20020924164928",
        "date": "2002-09-24",
        "lineage": "L2.1",
        "description": "메뉴 개편 · 배경 back.gif"
    },
    {
        "timestamp": "20021120053627",
        "date": "2002-11-20",
        "lineage": "L2.1",
        "description": "메뉴 개편 · 배경 back.jpg"
    },
    {
        "timestamp": "20021128181318",
        "date": "2002-11-28",
        "lineage": "L2.2",
        "description": "5분할 프레임 재설계"
    },
    {
        "timestamp": "20030726202839",
        "date": "2003-07-26",
        "lineage": "L3",
        "description": "싸이월드 연동형 마지막 판"
    }
];

export type FstoryAnnex = {
    id: string;
    label: string;
    period: string;
    snapshot: FstorySnapshotId;
    entry: string;
    summary: string;
};

// Earlier addresses the later editions never link back to. Offered as their own
// restore points so the recovered pages are reachable.
export const FSTORY_ANNEXES: FstoryAnnex[] = [
    {
        "id": "netian",
        "label": "네티앙 시절",
        "period": "1999-03 ~ 2001",
        "snapshot": "20011202212712",
        "entry": "netian/index__e97a8fd3.html",
        "summary": "fstory.net 도메인을 쓰기 전, my.netian.com/~fstory 에 있던 홈페이지. 첫 화면에 \"1999.3.17부터 방문자\"와 \"100% 메모장 노가다\" 라고 적혀 있다. L1 클래식 계보와 같은 title1~4.gif 이미지맵 메뉴를 쓴다."
    },
    {
        "id": "chollian-guestbook",
        "label": "천리안 방명록",
        "period": "2001 ~ 2002",
        "snapshot": "20021128181318",
        "entry": "chollian/cgi/pury/purybbs.html",
        "summary": "cgi.chollian.net/~foreststory 에 있던 PURY BBS 방명록. 목록 화면에 글 본문이 그대로 실리는 형식이라 오간 글과 답글이 남아 있다. 캡처마다 다른 쪽이 잡혀서 한 캡처만으로는 일부만 보이므로, 모든 캡처의 글을 번호로 합쳐 한 페이지로 다시 세웠다."
    }
];

export const FSTORY_SNAPSHOT_IDS = FSTORY_SNAPSHOTS.map((snapshot) => snapshot.timestamp);

export const lineageOf = (timestamp: string) =>
    FSTORY_LINEAGES.find((lineage) => lineage.id === FSTORY_SNAPSHOTS.find((snapshot) => snapshot.timestamp === timestamp)?.lineage);
